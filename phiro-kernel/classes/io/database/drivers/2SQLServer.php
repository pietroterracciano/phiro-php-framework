...
**/
class SQLServer extends \Phiro\IO\Database\Driver {
	...
	**/
    public function DBConnect() {
        ...
        $this->DBSocket->resource = sqlsrv_connect(
            $this->DBHost, // SERVER_NAME\INSTANCE_NAME, PORT_NUMBER (default is 1433)
            array( 
                'Database' => $this->DBName,
                'UID' => $this->DBUserName,
                'PWD' => $this->DBUserPassword
            )
        ); 
	...
	**/
    public function DBClose() {
        ...
        if( !@mysql_close($this->DBSocket->resource) ) {
            /**
            ...
            **/
            $this->setError(
                array(
                    'ID' => mysql_errno($this->DBSocket->resource),
                    'message' => mysql_error($this->DBSocket->resource)
                )
            );
        }
        return $this->afterDBClose();
    }
