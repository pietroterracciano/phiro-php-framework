...
$thrownBy = 'API_DB_DELETE_TABLE';

if(empty($_GET['DB_TABLE'])) new PhiroException('Database Table is empty', 'DTIE', $thrownBy, PhiroException::FAST_LAUNCH);
if(!$PhiroIoDatabase->query('DROP TABLE `'.$_GET['DB_TABLE'].'`')) new PhiroException($PhiroIoDatabase->getErrorMessage(), null, $thrownBy, PhiroException::FAST_LAUNCH);

echo PhiroUtilsJson::_encode(
    array(
        'STATUS' => 'Success',
    )
);
